...
# See script/rclone-xfer for full implementation and usage
alias rclonecopy='rclone-xfer copy'
alias rclonemove='rclone-xfer move'
...
alias zse='vi ~/.zshrc'
alias zgu='git -C $ZSH_DIR remote set-url origin '"$TIMEPIGGY_GIT_URL"'/zsh.git && git -C $ZSH_DIR pull origin master && git -C $ZSH_DIR submodule sync --recursive && git -C $ZSH_DIR submodule update --init --recursive && $ZSCRIPTS/run-migrations && sheldon lock --update && rm -f ~/.zcompdump*(N) && exec zsh'
