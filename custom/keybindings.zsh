# Zsh Keybindings
# Keybindings are defined in keybinding/keymap_zsh.yaml and interpreted by
# keybinding/interpret_zsh.py. The gen file is regenerated only when the YAML
# changes (mtime-gated), so normal shell starts pay zero interpreter overhead.

# ================================================
# Universal keymap loader (mtime-gated)
# ================================================
_zsh_yaml="$ZSH_CUSTOM/keybinding/keymap_zsh.yaml"
_zsh_gen="$ZSH_CUSTOM/keybinding/keybindings.gen.zsh"

if [[ -f $_zsh_yaml && ( ! -f $_zsh_gen || $_zsh_yaml -nt $_zsh_gen ) ]]; then
  local _zsh_tmp="${_zsh_gen}.tmp"
  if python3 "$ZSH_CUSTOM/keybinding/interpret_zsh.py" "$_zsh_yaml" > "$_zsh_tmp"; then
